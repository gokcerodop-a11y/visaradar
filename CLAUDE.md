...
**App Store durumu (2026-06-11, canlı API ile DOĞRULANDI):** v1.0.0 **build 4 WAITING_FOR_REVIEW** — uygulama + 3 IAP birlikte Apple inceleme kuyruğunda (submission 2a5b948f WAITING_FOR_REVIEW). 4. tur red (2.1(b) "Lifetime IAP'i bulamıyoruz" + 3.1.2(c) işlevsel EULA linki) **binary sorunuydu**, metadata değil: paywall'da (1) `_showLifetime` 3-gün gate Lifetime'ı gizliyordu, (2) "Terms" linki snackbar'dı (işlevsiz, Privacy yok). FIX = build 1.0.0+4: gate kaldırıldı (3 plan ilk açılışta görünür), Terms+Privacy işlevsel `launchUrl` linkleri, `AppConstants` URL'leri canlı worker'a set, worker'a `/support` eklendi+deploy, ASC ölü Support URL düzeltildi. IPA `xcrun altool --apiKey SDUZJJP88A` ile yüklendi (.p8 `~/.appstoreconnect/private_keys/`'te), build VALID→versiyona bağlandı→Notes+Contact API'den set→kullanıcı web'de **"Resubmit to App Review"**'a bastı→WAITING_FOR_REVIEW. KRİTİK SABİTLER: (a) .p8 API ilk-IAP'li versiyonu submit EDEMEZ, tek yol web "Resubmit" butonu; (b) "submitted" demeden ÖNCE mutlaka API'den WAITING_FOR_REVIEW doğrula; (c) telefona profile build ≠ App Store submit. Apple cevabı bekleniyor (~24-48h).
